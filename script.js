...
const VIEWS = ["dashboard", "organize", "notes", "docs", "calendar", "posts", "pillars", "ideas", "assets", "clients", "payroll", "tutorial", "backup", "about"];
...
const MORE_VIEWS = ["organize", "notes", "docs", "assets", "clients", "payroll", "tutorial", "backup", "about"];
...
function renderAssets() {
  ...
  state.cache.assets.forEach(function (a) {
    ...
    img.src = URL.createObjectURL(a.data);

    const card = el("button", { class: "assetCard", type: "button" }, [
      img,
      el("div", { class: "assetPill", text: Math.max(1, Math.round((a.size || 0) / 1024)) + " KB" }),
      el("div", { class: "assetCap", text: clampText(a.filename || "image", 36) })
    ]);
...
function openAssetPreview(assetId) {
  ...
  wrap.appendChild(el("div", { class: "notice" }, [
    el("div", { class: "itemTitle", text: a.filename || "image" }),
    el("div", { class: "muted", text: (a.mimeType || "") + " • " + Math.max(1, Math.round((a.size || 0) / 1024)) + " KB" })
  ]));
...
function openAssetPicker(postId) {
  ...
  if (!state.cache.assets.length) {
    ...
    state.cache.assets.forEach(function (a) {
      ...
      const card = el("button", { class: "assetCard", type: "button" }, [
        img,
        el("div", { class: "assetCap", text: clampText(a.filename || "image", 32) })
      ]);
...
async function openPillarEditor(pillarId) {
  let initial = { id: "", name: "", color: "#63ff98", notes: "" };
  if (pillarId) {
...
async function openIdeaEditor(ideaId) {
  let initial = { id: "", title: "", notes: "", tags: "" };
  if (ideaId) {
...
function openOnboardingFunnel(opts) {
  ...
  function renderStep() {
    ...
    if (key === "splash") {
      ...
      wrap.appendChild(el("div", { class: "notice" }, [
        el("div", { class: "itemTitle", text: "Handa ka na!" }),
        el("div", { class: "muted", text: "Mag-login o mag-sign up para masimulan ang totoong workspace mo. Ang lahat ng na-tour mo lang ay makikita ulit sa Tutorial tab, o buksan ulit ito sa hamburger menu (☰)." })
      ]));
    }
    ...
    if (stepIndex < steps.length - 1) {
      ...
      actions.appendChild(el("button", { class: "btn btnPrimary", type: "button", text: "Get Started", onclick: function () {
        setOnboardingDone();
        closeModal(true);
        enforceAccessUI();
      } }));
