...
const VIEWS = ["dashboard", "organize", "notes", "docs", "calendar", "posts", "pillars", "ideas", "assets", "clients", "meetings", "payroll", "tools", "tutorial", "backup", "about"];
const PREMIUM_VIEWS = ["calendar", "clients", "meetings", "payroll"];
...
// segmented sub-nav (#moreSegNav) that appears whenever one of them is active.
const MORE_VIEWS = ["organize", "notes", "docs", "assets", "clients", "meetings", "payroll", "tools", "tutorial", "backup", "about"];
...
function applyAppLanguage() {
  ...
  setText("navLblMeetings", dict.navMeetings || "Meetings");
...
function setView(view) {
  ...
  } else if (MORE_VIEWS.indexOf(view) !== -1) {
    const moreBtn = document.querySelector('.navItem[data-more="true"]');
    if (moreBtn) moreBtn.classList.add("active");
  }

  const moreSegNav = getEl("moreSegNav");
  if (moreSegNav) {
    if (MORE_VIEWS.indexOf(view) !== -1) {
      moreSegNav.classList.remove("hidden");
      $$(".segBtn", moreSegNav).forEach(function (b) { b.classList.toggle("active", b.dataset.seg === view); });
    } else {
      moreSegNav.classList.add("hidden");
    }
  }
...
// ---- Phase 10: Meetings tab (own nav item; reuses existing rooms/calls/Jitsi backend) ----
// The video-call engine (rooms table, invite codes, Jitsi embed) already existed inside the
// Clients tab — this phase just gives it its own dedicated "Meetings" home so a VA doesn't
// have to dig into a specific client to start or join a call, plus adds real room text chat
// (new "room_messages" table + Supabase Realtime, RLS via can_access_client()).

async function supabaseGetAllRooms() {
  if (typeof supabaseClient === "undefined") return [];
  const result = await supabaseClient
    .from("rooms")
    .select("*, clients(name)")
...
async function renderMeetings() {
  const list = getEl("meetingsList");
  if (!list) return;
  list.innerHTML = "";

  if (!hasVipFeatureAccess()) {
    list.appendChild(buildEmptyState("VIP lock", "Meetings is part of the VIP membership. Upgrade to unlock video calls and room chat."));
    return;
  }

  list.appendChild(buildEmptyState("Loading...", ""));
  const rooms = await supabaseGetAllRooms();
  list.innerHTML = "";

  if (!rooms.length) {
    list.appendChild(buildEmptyState("No meeting rooms yet", "Tap New Meeting to create a room, or Join Room if someone gave you a code."));
    return;
  }
    ...
    list.appendChild(el("div", { class: "item" }, [
      el("div", { class: "itemMain" }, [
        ...
        el("div", { class: "itemMeta" }, [
          el("span", { class: "badge", text: (r.clients && r.clients.name) ? r.clients.name : "No client" }),
      ...
      el("div", { class: "itemActions" }, [
        ...
        el("button", { class: "iconBtn", type: "button", text: "Chat", onclick: function () { openRoomChatModal(r); } }),
        el("button", { class: "btn btnSoft", type: "button", text: "Start Call", onclick: function () { startOrJoinCall(r); } })
      ])
    ]));
  });
}
...
function openNewMeetingModal() {
  ...
  if (!state.cache.clients.length) {
    alert("Gumawa muna ng Client (Clients tab) bago gumawa ng meeting room — kailangan ito ng bawat room.");
    return;
  }

  const clientSelect = el("select", { class: "select", id: "fMeetingClientId" },
    state.cache.clients.map(function (c) { return el("option", { value: c.id }, [c.name || "(Untitled client)"]); })
  );
  ...
  const wrap = el("div", { class: "formGrid" }, [
    ...
    el("div", { class: "fieldBlock" }, [
      el("div", { class: "label", text: "Room name" }),
      el("input", { class: "input", id: "fMeetingTitle", placeholder: "Example: Weekly Check-in" })
    ])
  ]);

  const actions = el("div", { class: "actionsRow" }, []);
  actions.appendChild(el("button", { class: "btn btnGhost", type: "button", text: "Cancel", onclick: function () { closeModal(true); } }));
  const createBtn = el("button", { class: "btn btnPrimary", type: "button", text: "Create Room" });
  createBtn.addEventListener("click", async function () {
    const clientId = getVal("fMeetingClientId", "");
    const title = (getVal("fMeetingTitle", "") || "").trim() || "Video Room";
    if (!clientId) { toast("Pumili muna ng client."); return; }
  ...
  openModal("New Meeting", wrap);
}

let roomChatChannel = null;

function closeRoomChatChannel() {
  if (roomChatChannel && typeof supabaseClient !== "undefined") {
    supabaseClient.removeChannel(roomChatChannel);
  }
  roomChatChannel = null;
}

async function openRoomChatModal(room) {
  const wrap = el("div", {}, []);
  wrap.appendChild(el("div", { class: "muted", text: "Room: " + (room.title || "Untitled room") }));

  const messagesBox = el("div", { class: "list", id: "roomChatMessages" }, [
    el("div", { class: "muted", text: "Loading messages..." })
  ]);
  wrap.appendChild(messagesBox);

  const composeRow = el("div", { class: "actionsRow" }, [
    el("input", { class: "input", id: "roomChatInput", placeholder: "Type a message..." }),
    el("button", { class: "btn btnPrimary", type: "button", id: "roomChatSendBtn", text: "Send" })
  ]);
  wrap.appendChild(composeRow);

  openModal("Room Chat", wrap, {});

  function renderMessages(rows) {
    messagesBox.innerHTML = "";
    if (!rows.length) {
      messagesBox.appendChild(el("div", { class: "muted", text: "Walang mensahe pa. Simulan ang usapan!" }));
      return;
    }
    rows.forEach(function (m) {
      messagesBox.appendChild(el("div", { class: "item" }, [
        el("div", { class: "itemMain" }, [
          el("div", { class: "muted", text: new Date(m.created_at).toLocaleString() }),
          el("div", { class: "itemTitle", text: m.body })
        ])
      ]));
    });
  ...
  const initial = await supabaseClient
    .from("room_messages")
    .select("*")
    .eq("room_id", room.id)
    .order("created_at", { ascending: true })
    .limit(200);
  renderMessages(initial.data || []);

  closeRoomChatChannel();
  roomChatChannel = supabaseClient
    .channel("room_messages_" + room.id)
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "room_messages", filter: "room_id=eq." + room.id }, function (payload) {
      const box = getEl("roomChatMessages");
      if (!box) return;
      const rows = Array.prototype.slice.call(box.children).length;
      box.appendChild(el("div", { class: "item" }, [
        el("div", { class: "itemMain" }, [
          el("div", { class: "muted", text: new Date(payload.new.created_at).toLocaleString() }),
          el("div", { class: "itemTitle", text: payload.new.body })
        ])
      ]));
  ...
  async function sendMessage() {
    const input = getEl("roomChatInput");
    const body = (input && input.value ? input.value : "").trim();
    if (!body) return;
    const user = await getCurrentUser();
    if (!user) return;
    input.value = "";
    const result = await supabaseClient.from("room_messages").insert([{ room_id: room.id, sender_id: user.id, body: body }]);
    if (result.error) { alert(result.error.message); }
  }

  bind("roomChatSendBtn", "click", sendMessage);
  setTimeout(function () {
    const input = getEl("roomChatInput");
...
function openOnboardingFunnel(opts) {
  const options = opts || {};
  const steps = ["splash", "tutorial", "roles", "language", "landing"];
  let stepIndex = 0;
...
function openHamburgerMenu() {
  ...
  const navList = el("div", { class: "list" }, destinations.map(function (d) {
    return el("button", { class: "item", type: "button", style: "width:100%;text-align:left;cursor:pointer", onclick: function () {
      closeModal(true);
      setView(d[0]);
    } }, [
      el("div", { class: "itemMain" }, [ el("div", { class: "itemTitle", text: d[1] }) ])
    ]);
  }));
  wrap.appendChild(navList);

  wrap.appendChild(el("div", { class: "actionsRow" }, [
    el("button", { class: "btn btnGhost", type: "button", text: "＋ Add", onclick: function () {
      closeModal(true);
      const b = getEl("btnQuickAdd");
      if (b) b.click();
    } }),
    el("button", { class: "btn btnGhost", type: "button", text: "⌕ Search", onclick: function () {
      closeModal(true);
      const b = getEl("btnSearch");
      if (b) b.click();
    } }),
    el("button", { class: "btn btnGhost", type: "button", text: "⬒ Backup", onclick: function () {
      closeModal(true);
      const b = getEl("btnBackup");
      if (b) b.click();
    } })
  ]));

  wrap.appendChild(el("div", { class: "actionsRow" }, [
    el("button", { class: "btn btnSoft", type: "button", text: "Paano Gamitin (Tutorial Tour)", onclick: function () {
      closeModal(true);
      openOnboardingFunnel({ force: false });
    } }),
    el("button", { class: "btn btnGhost", type: "button", text: "Account / Login", onclick: function () {
      closeModal(true);
      openAuthModal({ force: false });
    } })
  ]));

  openModal("Menu", wrap, { locked: false });
}
...
function openSearch() {
  ...
  function run() {
    ...
    results.innerHTML = "";
    if (!q) {
      results.appendChild(buildEmptyState("Type to search", "Search posts, tasks, and docs."));
      return;
    }
    ...
    if (!posts.length && !tasks.length && !docs.length) {
...
function wireUI() {
  bind("btnHamburger", "click", function () { openHamburgerMenu(); });
  ...
  bind("btnBackup", "click", function () { setView("backup"); });
  ...
  bind("meetingsJoinBtn", "click", function () { openJoinRoomModal(); });
  bind("meetingsNewBtn", "click", function () { openNewMeetingModal(); });
