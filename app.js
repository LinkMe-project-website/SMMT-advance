...
function closeTopMenu() { $("topMenuOverlay").classList.add("hidden"); }
...
async function loadFollowStats() {
  ...
  const [{ count: followersCount }, { count: followingCount }] = await Promise.all([
    supabaseClient.from("followers").select("follower_id", { count: "exact", head: true }).eq("following_id", currentUser.id),
    supabaseClient.from("followers").select("follower_id", { count: "exact", head: true }).eq("follower_id", currentUser.id),
  ]);
...
async function createOrStartChat(userId) {
  ...
  // (meetings has no "other participant" column — chatThreadsCache only
  // has created_by. So we look at meeting_participants for the real
  // membership of every chat-status thread I'm already in.)
  const candidateThreads = chatThreadsCache.filter(t => t.status === "chat");
  if (candidateThreads.length) {
    const { data: parts } = await supabaseClient
      .from("meeting_participants")
      .select("room_id, user_id")
      .in("room_id", candidateThreads.map(t => t.id));
    const membersByRoom = {};
    (parts || []).forEach(p => {
      (membersByRoom[p.room_id] ||= new Set()).add(p.user_id);
    });
    const existing = candidateThreads.find(t => {
      const members = membersByRoom[t.id] || new Set();
      members.add(t.created_by);
