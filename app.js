...
// Navigation
// ---------------------------------------------------------------------------
const VIEW_TITLES = { dashboard: "Home", meetings: "Meetings", chat: "Chats", recordings: "Recordings", notifications: "Notifications", whiteboard: "Whiteboard", profile: "Profile", more: "Community", vip: "VIP Membership" };
...
function setActiveView(view) {
  ...
  if (view === "profile") { $("pageSubtitle").textContent = "Account & settings"; loadMyPosts(); }
...
function renderProfile() {
  ...
  $("profileHeaderName").textContent = p.full_name || "Welcome";
...
async function openFollowListModal(type) {
  ...
  const { data: profilesData } = await supabaseClient.from("profiles").select("id, full_name, avatar_url, role").in("id", ids);
  ...
  body.innerHTML = profilesData.map(u => {
    ...
    return `<div class="listItem">
      ...
      <div style="flex:1;min-width:0">
        <div class="itemTitle">${escapeHtml(u.full_name || "Unnamed")}</div>
...
async function loadFeed() {
  ...
  if (error) { targets.forEach(l => l.innerHTML = `<div class="emptyState">Could not load feed.</div>`); return; }
  ...
  if (!data || !data.length) {
    targets.forEach(l => l.innerHTML = `<div class="emptyState">No announcements yet — check back soon.</div>`);
    return;
  }
  const makeCard = (p) => `
  ...
  `;
  targets.forEach(l => {
    l.innerHTML = (l.id === "dashFeedList" ? data.slice(0, 5) : data).map(makeCard).join("");
  });
}
...
async function loadMarketplace() {
      ...
      <div class="mkCard" data-mk-id="${l.id}">
        ...
        <div class="mkInfo">
          ...
          <div class="mkSellerRow">
            <span class="mkSellerName">${escapeHtml(l.poster_name || "Seller")}</span>
  ...
  grid.querySelectorAll("[data-mk-id]").forEach(card => {
    ...
    card.addEventListener("click", () => openMarketplaceDetail(item));
  });
}
...
if ($("btnPostListing")) {
  $("btnPostListing").addEventListener("click", () => {
    openModal(`
      ...
      <div class="field"><label>Photo URL (optional)</label><input type="text" id="listPhotoInput" placeholder="https://... (paste image link)" /></div>
    ...
    $("listSubmit").addEventListener("click", async () => {
      ...
      const photo = $("listPhotoInput").value.trim();
...
// backward compat
async function loadJobs() { await loadMarketplace(); }
...
async function loadForum() {
  ...
  list.innerHTML = visible.map(f => {
    ...
    <div class="rumbleCard${f.is_pinned ? " rumbleCard--pinned" : ""}" data-thread="${f.id}">
      ...
      <div class="rumbleFoot">
        <span class="rumbleAuthor">${escapeHtml(f.author_name || "Guest")} ${authorRole ? getRoleBadgeHtml(authorRole) : ""}</span>
  ...
  list.querySelectorAll(".rumbleCard").forEach(card => {
    card.addEventListener("click", () => { const post = visible.find(f => f.id === card.dataset.thread); openThreadReplies(card.dataset.thread, post); });
  });
}
...
function openThreadReplies(postId, post) {
  ...
  openModal(`
    ...
    <div class="rumbleModalMeta">${escapeHtml(post.author_name || "Guest")} ${authorRole ? getRoleBadgeHtml(authorRole) : ""} &bull; ${fmtDate(post.created_at)}</div>
  ...
  (async () => {
    ...
    replyList.innerHTML = data.map(r => `
      <div class="replyCard">
        <span class="rumbleAuthor">${escapeHtml(r.author_name || "Guest")}</span>
