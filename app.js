...
async function openProfileView(userId) {
  ...
  $("profileViewBody").innerHTML = `
    ...
    <div class="pvHeaderWrap">
      <div class="pvNameRow">
        <div class="profileViewAvatar">${p.avatar_url ? `<img loading="lazy" src="${escapeHtml(p.avatar_url)}" alt="" style="cursor:pointer" data-lightbox="${escapeHtml(p.avatar_url)}"/>` : initial}</div>
        <div class="pvNameCol">
...
async function loadWorkspace() {
  ...
  if (wsActiveTab === "your-videos") {
    ...
    if (error || !data.length) {
      list.innerHTML = `<div class="emptyState">No videos yet — check back soon!</div>`;
      return;
    }
    list.innerHTML = data.map(v => `
      <div class="wsVideoCard" data-vid-id="${v.id}">
        ...
        <div class="wsVideoInfo">
          ...
          <div class="itemMeta">${nameLink(v.uploader_name || "Creator", getUserVipRow(v.uploaded_by), v.uploaded_by)} • 👁 ${v.view_count || 0} • ❤️ ${v.likes_count || 0}</div>
        </div>
      </div>`).join("");
