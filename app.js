...
function renderProfile() {
  ...
  // Topbar profile avatar: show photo if available, else initials
  const topBtn = $("btnProfileTop");
  if (p.avatar_url) {
...
async function openProfileView(userId) {
  ...
  const [{ data, error }, { data: ratingData }, { data: reviewsData }, { data: followRow }, { data: myReview }, { data: badgeRow }] = await Promise.all([
  ...
  $("profileViewBody").innerHTML = `
    <div class="pvCover">
      ${p.cover_photo_url ? `<img loading="lazy" class="pvCoverImg" src="${escapeHtml(p.cover_photo_url)}" alt=""/>` : ""}
    </div>
    <div class="pvHeaderWrap">
      <div class="pvNameRow">
        <div class="profileViewAvatar">${p.avatar_url ? `<img loading="lazy" src="${escapeHtml(p.avatar_url)}" alt=""/>` : initial}</div>
        <div class="pvNameCol">
      ...
      <div class="pvActionRow">
        <button class="btn btnPrimary" id="btnMessageUser">💬 Message</button>
...
function communityCardHtml(c, myRole) {
  return `
    <div class="commCard" data-comm-open="${c.id}">
      ...
      ${myRole
        ? `<div class="commMeta" style="margin-top:6px">${myRole === "owner" ? "👑 Owner" : myRole === "admin" ? "🛡️ Admin" : "✓ Member"}</div>`
    ...
    <div id="commMembersList" style="max-height:280px;overflow-y:auto;margin-bottom:16px">
      ${memberRows.map(m => {
        ...
        <div class="commMemberRow">
          ...
          <span class="commRoleBadge${m.role === "owner" ? " commRoleBadge--owner" : ""}">${m.role === "owner" ? "👑 Owner" : m.role === "admin" ? "🛡️ Admin" : "Member"}</span>
          ${(isManager && m.user_id !== currentUser.id && m.role !== "owner") ? `<button class="partRemoveBtn" style="background:rgba(239,68,68,.1);color:#b91c1c" data-comm-remove-member="${m.user_id}">Remove</button>` : ""}
        </div>`;
      }).join("")}
    </div>
...
async function loadCommunityFeed(communityId) {
  ...
  if (!el) return;
  const [{ data: posts }, { data: listings }, { data: vids }] = await Promise.all([
    supabaseClient.from("forum_posts").select("id, title, author_name, created_at").eq("community_id", communityId).order("created_at", { ascending: false }).limit(20),
    supabaseClient.from("marketplace_listings").select("id, title, price, poster_name, created_at").eq("community_id", communityId).order("created_at", { ascending: false }).limit(20),
    supabaseClient.from("videos").select("id, title, created_at").eq("community_id", communityId).eq("flagged", false).order("created_at", { ascending: false }).limit(20),
  ]);
  const items = [
    ...(posts || []).map(p => ({ kind: "post", icon: "📝", label: p.title || "Untitled post", by: p.author_name, id: p.id, created_at: p.created_at })),
    ...(listings || []).map(l => ({ kind: "listing", icon: "🛍️", label: l.title, by: l.poster_name, id: l.id, created_at: l.created_at, listing: l })),
    ...(vids || []).map(v => ({ kind: "video", icon: "🎥", label: v.title, by: null, id: v.id, created_at: v.created_at })),
  ].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
...
async function loadAdminAnalytics() {
  ...
  const { data: topListings } = await supabaseClient.from("marketplace_listings")
    .select("id, title, listing_type, price, view_count")
  ...
  if (adminTopListings && topListings?.length) {
    adminTopListings.innerHTML = topListings.map(l => `
      <div class="listItem">
        <div style="flex:1;min-width:0">
          <div class="itemTitle">${escapeHtml(l.title)}</div>
          <div class="itemMeta">${l.listing_type || "listing"} • ${l.price ? `₱${Number(l.price).toLocaleString()}` : "—"} • 👁 ${l.view_count || 0} views</div>
        </div>
      </div>`).join("");
...
async function renderMarketplaceListings(listings, ownerView) {
  ...
  grid.querySelectorAll("[data-mk-fill]").forEach(btn => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
...
if (videoFileInput) {
  videoFileInput.addEventListener("change", async (e) => {
    ...
    $("vidSubmit").addEventListener("click", async () => {
      ...
      closeModal();
      showToast(finalMod.flagged ? "Posted - pending review." : "🎬 Video uploaded!");
      loadWorkspace();
    });
  });
}
  ...
  if (wsActiveTab === "your-videos") {
    ...
    list.innerHTML = data.map(v => `
      <div class="wsVideoCard" data-vid-id="${v.id}">
        <div class="wsVideoThumb">${v.thumbnail_url ? `<img loading="lazy" src="${escapeHtml(v.thumbnail_url)}" />` : `<div class="wsVideoThumbPlaceholder">🎬</div>`}</div>
        <div class="wsVideoInfo">
          <div class="wsVideoTitle">${escapeHtml(v.title)}${v.flagged ? ` <span class="badge" style="color:var(--danger);font-size:10px">Under review</span>` : ""}</div>
    ...
    // Hide videos still pending moderation review from everyone but their uploader.
    const data = (rawData || []).filter(v => !v.flagged || v.uploaded_by === currentUser.id);
    if (error || !data || !data.length) {
...
Return: {"type": "people"|"marketplace"|"forum"|"videos"|"all", "keywords": ["word1","word2"], "maxPrice": null|number, "location": null|string, "category": null|string, "naturalSummary": "brief human-readable interpretation"}`,
    ...
    if (filterType === "all" || filterType === "marketplace") {
      let mkQ = supabaseClient.from("marketplace_listings").select("id, title, price, listing_type").ilike("title", `%${kw}%`);
      if (parsedIntent?.maxPrice) mkQ = mkQ.lte("price", parsedIntent.maxPrice);
      if (parsedIntent?.category) mkQ = mkQ.eq("listing_type", parsedIntent.category);
    ...
    if (filterType === "all" || filterType === "marketplace") {
      ...
      if (mk?.length) sections.push(`<div class="itemMeta" style="margin:8px 0 4px">🛍️ Marketplace</div>` + mk.map(j => `<div class="listItem"><div><div class="itemTitle">${escapeHtml(j.title)}</div><div class="itemMeta">${j.listing_type || "item"}${j.price ? ` • ₱${Number(j.price).toLocaleString()}` : ""}</div></div></div>`).join(""));
    }
...
async function loadCommunityRecommendations() {
  ...
  if (mySkills.length) {
    ...
    const { data: jobMatches } = await supabaseClient.from("marketplace_listings")
      .select("id, title, price, listing_type, location, created_at")
      .or(mySkills.slice(0, 3).map(s => `title.ilike.%${s}%`).join(","))
      .in("listing_type", ["job", "service"])
      .order("created_at", { ascending: false })
      .limit(5);
    recoJobs = jobMatches || [];
  }
  ...
  if (!recoJobs.length) {
    ...
    recoJobsEl.innerHTML = `<div style="display:flex;flex-direction:column;gap:8px">` + recoJobs.map(j => `
      <div style="display:flex;align-items:center;gap:10px;padding:10px;border:1px solid var(--border);border-radius:14px;background:var(--panel2)">
        <div style="font-size:22px;flex-shrink:0">${j.listing_type === "service" ? "🔧" : "💼"}</div>
        <div style="flex:1;min-width:0">
