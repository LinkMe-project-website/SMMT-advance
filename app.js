...
function renderProfile() {
  ...
  if (p.avatar_url) {
    topBtn.innerHTML = `<img src="${escapeHtml(p.avatar_url)}" alt="${escapeHtml(initials)}" />`;
  } else {
...
async function openProfileView(userId) {
  ...
  $("profileViewBody").innerHTML = `
    <div class="pvCover">
      ${p.cover_photo_url ? `<img class="pvCoverImg" src="${escapeHtml(p.cover_photo_url)}" alt=""/>` : ""}
    </div>
    <div class="pvHeaderWrap">
      <div class="pvNameRow">
        <div class="profileViewAvatar">${p.avatar_url ? `<img src="${escapeHtml(p.avatar_url)}" alt=""/>` : initial}</div>
        <div class="pvNameCol">
