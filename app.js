  ...
  list.innerHTML = combined.map(item => {
    ...
    return `
      <div class="dashFeedCard" data-dash-mk="${item.id}" style="cursor:pointer">
        ...
        <div class="dashFeedMeta">${fmtDate(item.created_at)} · ${renderVipName(item.poster_name || "Seller", getUserVipRow(item.posted_by))}</div>
      </div>`;
  }).join("");

  list.querySelectorAll("[data-dash-mk]").forEach(card => {
    const item = visibleMk.find(l => l.id === card.dataset.dashMk);
    if (item) card.addEventListener("click", () => openMarketplaceDetail(item));
  });
...
if (videoFileInput) {
  videoFileInput.addEventListener("change", async (e) => {
    ...
    $("vidSubmit").addEventListener("click", async () => {
      ...
      const btn = $("vidSubmit");
      btn.disabled = true; btn.textContent = "Uploading…";
      $("vidUploadProgress").textContent = "Uploading video, please wait…";
      const ext = (file.name.split(".").pop() || "mp4").toLowerCase();
      const path = `${currentUser.id}/videos/${Date.now()}.${ext}`;
      const { error: upErr } = await supabaseClient.storage.from("assets").upload(path, file, { upsert: false });
      if (upErr) { showToast("Upload failed: " + upErr.message); btn.disabled = false; btn.textContent = "Upload"; return; }
      const { data: urlData } = supabaseClient.storage.from("assets").getPublicUrl(path);
  ...
  if (wsActiveTab === "your-videos") {
    ...
    list.innerHTML = data.map(v => `
      <div class="wsVideoCard" data-vid-id="${v.id}">
        ...
        <div class="wsVideoInfo">
          <div class="wsVideoTitle">${escapeHtml(v.title)}</div>
          <div class="itemMeta">${renderVipName(v.uploader_name || "Creator", getUserVipRow(v.uploaded_by))} • 👁 ${v.view_count || 0}</div>
        </div>
      </div>`).join("");
    list.querySelectorAll("[data-vid-id]").forEach(card => {
      const vid = data.find(v => v.id === card.dataset.vidId);
...
function openVideoPlayer(v) {
  const { data: urlData } = supabaseClient.storage.from("assets").getPublicUrl(v.file_path);
