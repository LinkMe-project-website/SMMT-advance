      ...
      try {
        ...
        resultsEl.innerHTML = results.map(p => `
          <div class="contactItem">
            <div>
              <div class="contactName">${renderVipName(p.full_name || "—", p)}</div>
              <div class="contactMeta">MG ID ${escapeHtml(p.mg_id || "—")}${getVipTier(p) ? " • VIP" : ""}</div>
            </div>
      ...
      <div class="mkCard" data-mk-id="${l.id}">
        ...
        <div class="mkInfo">
          ...
          <div class="mkSellerRow">
            <span class="mkSellerName">${renderVipName(l.poster_name || "Seller", getUserVipRow(l.posted_by))}</span>
  ...
  list.innerHTML = visible.map(f => {
    ...
    <div class="rumbleCard${f.is_pinned ? " rumbleCard--pinned" : ""}" data-thread="${f.id}">
      ...
      <div class="rumbleFoot">
        <span class="rumbleAuthor">${renderVipName(f.author_name || "Guest", getUserVipRow(f.author_id))} ${authorRole ? getRoleBadgeHtml(authorRole) : ""}</span>
...
function openThreadReplies(postId, post) {
  ...
  openModal(`
    ...
    <div class="rumbleModalMeta">${renderVipName(post.author_name || "Guest", getUserVipRow(post.author_id))} ${authorRole ? getRoleBadgeHtml(authorRole) : ""} &bull; ${fmtDate(post.created_at)}</div>
  ...
  (async () => {
    ...
    replyList.innerHTML = data.map(r => `
      <div class="replyCard">
        <span class="rumbleAuthor">${renderVipName(r.author_name || "Guest", getUserVipRow(r.author_id))}</span>
