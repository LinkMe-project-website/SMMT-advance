...
// Navigation
// ---------------------------------------------------------------------------
const VIEW_TITLES = { dashboard: "Dashboard", meetings: "Meetings", chat: "Chat", whiteboard: "Whiteboard", profile: "Profile" };
...
function setActiveView(view) {
  ...
  $("btnSettingsTop").classList.toggle("hidden", view !== "profile");
  if (view === "dashboard") updateDashboardSubtitle();
  else if (view === "profile") $("pageSubtitle").textContent = "Account & settings";
...
function renderDashboard() {
  ...
  $("dashName").innerHTML = `${greeting} 👋<br/><span style="font-size:13px;color:var(--muted);font-weight:400">${today}</span>`;
  if (document.querySelector('.navTab[data-view="dashboard"]')?.classList.contains("active")) updateDashboardSubtitle();
...
// Opens a call (video meeting or voice-only) using the Jitsi Meet External API,
// with our own green control bar instead of Jitsi's built-in toolbar
// (Jitsi's own toolbar is hidden via toolbarButtons:[] / TOOLBAR_BUTTONS:[]).
function openJitsiCall({ room, title, audioOnly, showCamBtn }) {
  ...
  // If the user ends the call from inside the Jitsi UI itself (e.g. browser
  // back/refresh triggers Jitsi's own cleanup), make sure our UI + DB state
  // stay in sync too.
...
async function createOrStartChat(userId) {
  ...
  const { data: room, error } = await supabaseClient.from("meetings").insert({
    title: `Chat with ${profile?.full_name || "User"}`,
    status: "chat",
    created_by: currentUser.id,
  }).select().single();
...
$("btnNewGroupChat").addEventListener("click", () => {
  ...
  $("createGroupBtn").addEventListener("click", async () => {
    ...
    const participants = [{ room_id: room.id, user_id: currentUser.id, role: "member" }, ...selected.map(uid => ({ room_id: room.id, user_id: uid, role: "member" }))];
