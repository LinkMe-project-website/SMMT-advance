...
async function loadMeetings() {
  // Phase 9 perf: this query previously had no limit at all — capped at 300
  // (most-recently-scheduled first isn't safe to change here since dashboard/
  // upcoming-meeting logic elsewhere assumes ascending order, so a full
  // "Load more" UI for Chats needs a proper split of meetings vs. chat-thread
  // queries — left as a follow-up; this cap just stops unbounded growth).
  const { data, error } = await supabaseClient.from("meetings").select("*").order("scheduled_at", { ascending: true }).limit(300);
  if (error) { console.error(error); return; }
  meetingsCache = data || [];
  renderMeetings();
  renderDashboard();
...
$("btnCreateCommunity").addEventListener("click", () => {
  ...
  $("commCreateSubmit").addEventListener("click", async () => {
    ...
    if (!name) { showToast("Please name your group."); return; }
...
async function openCommunityDetail(communityId) {
  ...
  $("modalCard").innerHTML = `
    ...
    <div id="commMembersList" style="max-height:280px;overflow-y:auto">
      ${memberRows.map(m => {
        const p = m.profiles || {};
        const initial = escapeHtml((p.full_name || "?").charAt(0).toUpperCase());
        return `
...
function renderMarketplaceListings(listings, ownerView) {
  const grid = $("mkGrid");
  if (!grid) return;
