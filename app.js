...
function joinVoiceCall(callId, roomId) {
  ...
  $("callTitleText").textContent = "Voice call — " + (room?.title || "");
  $("callIframe").src = `https://meet.jit.si/${encodeURIComponent(jitsiRoom)}#userInfo.displayName="${encodeURIComponent(currentProfile?.full_name || "Guest")}"&config.startAudioOnly=true&config.startWithVideoMuted=true&config.disableInviteFunctions=true`;
  $("callFrameWrap").classList.remove("hidden");
}
