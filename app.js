  ...
  if (tab === "posts") {
    ...
    body.querySelectorAll("[data-pv-like]").forEach(btn => btn.addEventListener("click", async () => {
      ...
      if (myLikedSet.has(postId)) {
        ...
        if (!error) await supabaseClient.from("forum_posts").update({ likes_count: curCount + 1 }).eq("id", postId);
      }
      loadProfileViewTab(userId, "posts");
    }));
    ...
    const [{ data: mkData }, { data: rbData }, { data: vidData }] = await Promise.all([
      ...
      supabaseClient.from("videos").select("thumbnail_url").eq("uploaded_by", userId),
    ]);
    const photos = [];
    (mkData || []).forEach(l => (l.photos || []).forEach(p => { if (p) photos.push(p); }));
    ...
    const { data: vids } = await supabaseClient.from("videos").select("*").eq("uploaded_by", userId).order("created_at", { ascending: false });
    ...
    body.querySelectorAll("[data-pv-vid-like]").forEach(btn => btn.addEventListener("click", async () => {
      ...
      if (myVidLikes.has(videoId)) {
        ...
        await supabaseClient.from("video_likes").insert({ video_id: videoId, user_id: currentUser.id });
      }
...
async function forwardMessageToRoom(msg, targetRoomId) {
  ...
  notifyOtherParticipants(targetRoomId, "chat_message", currentProfile?.full_name || "Someone", "Forwarded a message to you");
...
async function stopVoiceRecordingAndSend() {
  ...
  voiceRecorder.onstop = async () => {
    ...
    try {
      ...
      notifyOtherParticipants(roomId, "chat_message", currentProfile?.full_name || "Someone", "Sent a voice note");
    } catch (err) {
...
function openMarketplaceDetail(l) {
  ...
  $("mkMsgSeller") && $("mkMsgSeller").addEventListener("click", async () => {
    ...
    closeModal();
    setActiveView("chat");
    showToast(isJob ? "Opening chat... find the employer in contacts." : "Opening chat... find the seller in contacts.");
  });
